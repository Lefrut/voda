...
class ServiceDetailViewModel @Inject constructor(
...
) : MviViewModel<ServiceDetailState, ServiceDetailEvent>(ServiceDetailState()) {
    ...
    fun fetchServiceDetails() = viewModelScope.launch {
        _state.update { s ->
            s.copy(uiState = ServiceDetailUiState.Loading)
        }
        ...
        serviceDetailsResult.onSuccess { serviceDetails ->
            ...
            _state.update { s ->
                s.copy(
                    uiState = ServiceDetailUiState.Success,
            ...
            _state.update { s ->
                s.copy(uiState = ServiceDetailUiState.Error)
            }
        }
    }

    fun navigateBack() = viewModelScope.launch {
        _events.emit(ServiceDetailEvent.GoBack)
    }

    fun navigateToProductDetails(product: ProductUi) = viewModelScope.launch {
        _events.emit(ServiceDetailEvent.GoToProductDetails(product.id))
    }
    ...
    fun navigateToAnalogs(product: ProductUi) = viewModelScope.launch {
        _events.emit(ServiceDetailEvent.GoToAnalogs(product.id))
    }
